package com.juntai.wisdom.im.chat_module.voip;
...
import com.juntai.disabled.basecomponent.permission.Permissions;
import com.juntai.disabled.basecomponent.utils.ActivityStackManager;
...
import com.juntai.wisdom.im.utils.Utils;
...
public class VoipReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Constant.ACTION_VOIP_RECEIVER.equals(action)) {
    ...
    private void onBackgroundAfterVersionO(
        ...
        if (hasPermission) {
            ...
            CallForegroundNotification notification = new CallForegroundNotification(MyApp.app);
    ...
    private void onBackgroundHasPermission(
            ...
            String inviteId, Boolean audioOnly, String inviteUserName) {
        boolean b = SkyEngineKit.Instance().startInCall(MyApp.app, room, inviteId, audioOnly);
        LogUtils.dTag(TAG, "onBackgroundHasPermission b = " + b );
        if (b) {
            // TODO: 2022/4/12 原代码 先注释掉
//            MyApp.app.setOtherUserId(inviteId);
            if (list.size() == 1) {
                CallForegroundNotification notification = new CallForegroundNotification(MyApp.app);
...
//            MyApp.app.setOtherUserId(inviteId);
            ...
            if (list.size() == 1) {
                ...
                if (context instanceof SingleCallActivity) {
                    ((SingleCallActivity) context).finish();
                }
                SingleCallActivity.openActivity(context, inviteId, false, inviteUserName, audioOnly, true);
            } else {
                // 群聊
            }
        } else {
    ...
    private void onPermissionDenied(String room, String inviteId) {
        ...
        Toast.makeText(MyApp.app, "权限被拒绝，无法通话", Toast.LENGTH_SHORT).show();
    }

    private void shouldStartRing(boolean isComing) {
        if (isComing) {
            Uri uri = Uri.parse("android.resource://" + MyApp.app.getPackageName() + "/" + R.raw.incoming_call_ring);
            ringPlayer.play(MyApp.app, uri, true, AudioManager.STREAM_RING);
        } else {
            Uri uri = Uri.parse("android.resource://" + MyApp.app.getPackageName() + "/" + R.raw.wr_ringback);
            ringPlayer.play(MyApp.app, uri, true, AudioManager.STREAM_RING);
        }
    }
